...
class TextClassificationDataset:
    ...
    def __init__(self, cfg, partition):
        ...
        if self.cfg['prepro'] == 'None':
            self.target_path = f"data/{self.cfg['data']}-{self.cfg['pretrained'].replace('/', '_')}.h5"
        else:
            self.target_path = f"data/{self.cfg['data']}-{self.cfg['pretrained'].replace('/', '_')}-prepro_{self.cfg['prepro']}.h5"
            
        f = h5py.File(self.target_path, "r")
        attrs = f["attributes"]
        self.indices = attrs[f"{partition}_ind"][()]
        ...
        self.y_true = attrs[f"{self.partition}_labels"][()]

    def split(self, new_partition, ratio):
        data = TextClassificationDataset(self.cfg, self.partition)
        self.indices, data.indices, self.y_true, data.y_true = train_test_split(self.indices, self.y_true, shuffle=True, random_state=np.random.RandomState(61), test_size=ratio)
