...
def class_avg_finetune(model, texts, labels, nb_classes, batch_size,
                       method, epoch_size=5000, nb_epochs=1000, error_checking=True,
                       verbose=True, lr=0.0001, next_lr=0.0001):
    ...
    if method == 'chain-thaw':
        result, all_y_pred_val, all_y_pred_test, all_ts = class_avg_chainthaw(model, nb_classes=nb_classes,
        ...
        result = class_avg_tune_trainable(model, nb_classes=nb_classes,
                                          ...
                                          checkpoint_weight_path=checkpoint_path,
                                          initial_lr=lr, next_lr=next_lr,
                                          verbose=verbose)
    return model, result, all_y_pred_val, all_y_pred_test, all_ts
...
def class_avg_chainthaw(model, nb_classes, train, val, test, batch_size,
    ...
    for i in range(nb_iter):
        ...
        if all_y_pred_val is None:
            all_y_pred_val = y_pred_val
        else:
            all_y_pred_val = np.concatenate((all_y_pred_val, y_pred_val), axis=1)
            
        if all_y_pred_test is None:
            all_y_pred_test = y_pred_test
        else:
            all_y_pred_test = np.concatenate((all_y_pred_test, y_pred_test), axis=1)
            
        all_ts.append(best_t)
            
        print(all_y_pred_val)
        print(all_ts)

    return total_f1 / nb_iter, all_y_pred_val, all_y_pred_test, all_ts
